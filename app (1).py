...
def init_db():
    ...
    # Garante que o admin sempre existe com a senha padrão
    ...
    if not existe:
        ...
        conn.execute(
            "UPDATE usuarios SET senha=? WHERE login='admin'",
            (hash_senha("admin123"),)
        )
    conn.commit()
    conn.close()
...
def login():
    ...
    if request.method == "POST":
        login_str = request.form["login"].strip().lower()
        senha = hash_senha(request.form["senha"])
        conn = get_db()
        user = conn.execute("SELECT * FROM usuarios WHERE login=? AND senha=?", (login_str, senha)).fetchone()
        conn.close()
        if user:
...
def dashboard():
    ...
    """).fetchall()
    conn.close()
    return render_template("dashboard.html", totais=totais, mensal=mensal)
...
def novo_usuario():
    ...
    conn = get_db()
    try:
        conn.execute("INSERT INTO usuarios (nome, login, senha, perfil) VALUES (?,?,?,?)", (nome, login_str, senha, perfil))
        conn.commit()
